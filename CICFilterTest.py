...
filename="C:\\Users\\arg\\Documents\\MATLAB\\pdm_sine_wave_11.txt"
...
with open(filename, 'r') as file:
    ...
    while(line!=""):
        ...
        out_array.append(int(line.strip()))
...
fig, ax = plt.subplots()
ax.plot(xf[0:600], (2.0/N * np.abs(yf[:N//2]))[0:600])
plt.show()
# Display the plot
# plt.show()
...
from scipy.io.wavfile import write,read
import soundfile as sf
# filename="C:\\Users\\arg\\Documents\\MATLAB\\pdm_sine_wave_8.txt"
...
buffer=np.zeros(10241)

with open(filename, 'r') as file:
    line=file.readline()
    # for i in range(len(buffer)):
    #     buffer[i]=int(line.strip())
    #     line=file.readline()
    while(line!=""):
        # # print(buffer.sum())
        # int1=twos_complement_addition(int1,decimal_to_binary(int((np.sum(buffer)))))
        
        # int2=twos_complement_addition(int1,int2)
        # int3=twos_complement_addition(int2,int3)
        
        buffer=np.roll(buffer,1)
        buffer[0]=int(line.strip())
        line=file.readline()
        out_array.append(int((np.sum(buffer))))
...
print(np.sum(np.square(nparr)))
print("hereq2")
write("ExtraMics16/AudioTests/cictestp.wav", 48000,scipy.signal.decimate(nparr,64))
plt.plot(nparr)
# plt.show()
import scipy
# Add labels and title for better understanding
